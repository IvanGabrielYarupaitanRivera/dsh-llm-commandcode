// types.ts — CommandCode wire vocabulary and the plugin's configuration shape.
...
export type CcMessage =
  | { role: 'user'; content: string | CcTextBlock[] }
  | { role: 'assistant'; content: CcContentBlock[] }
  | { role: 'tool'; content: [CcToolResultBlock] }
...
export type CcEvent =
  ...
  | { type: 'tool-result' }
  | { type: 'finish'; finishReason?: string; totalUsage?: CcUsage }
  | { type: 'finish-step' }
