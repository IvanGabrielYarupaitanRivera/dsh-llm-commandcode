// config.js — schema and normalization for the llm-commandcode plugin.
//
...
const REASONING_EFFORTS = new Set(['off', 'high', 'max'])

/**
 * Validate a raw configuration object and produce the frozen facts the adapter
 * reads. Throws on values the adapter cannot serve; never silently fixes them.
 */
export function normalizeConfig(raw = {}) {
  const config = {
    ...
    models: Array.isArray(raw.models) && raw.models.length > 0 ? raw.models : DEFAULT_MODELS,
    ...
    retryPolicy: raw.retryPolicy !== undefined ? resolveRetryPolicy(raw.retryPolicy) : undefined,
  }
  ...
  config.models = config.models.filter((entry) => {
    if (!entry || typeof entry.id !== 'string' || entry.id.length === 0 || seen.has(entry.id)) return false
    seen.add(entry.id)
    return true
  })
