...
export function normalizeConfig(raw = {}) {
    const config = {
        ...
        models: raw.models !== undefined && raw.models.length > 0 ? [...raw.models] : [...schemaDefaultModels],
    ...
    if (config.reasoningEffort !== undefined && !REASONING_EFFORTS.has(config.reasoningEffort)) {
        throw new Error(`llm-commandcode: reasoningEffort must be one of off, high, max (got "${config.reasoningEffort}")`);
    }
    const seen = new Set();
